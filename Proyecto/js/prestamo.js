...
function recuperarPrestamo(){
    ...
    if (localStorage.getItem("prestamos") != null){
        ...
        //si no hay nada guardado retorno null
        return null;
    }
}
...
function cotizacion(){
    ...
    const url = new Request('http://127.0.0.1:5500/PrimeraEntrega-JavaScript-main/js/cotizaciones.json');
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        for(let i = 0; i < data.Cotizaciones.length; i++) {
          
            if (data.Cotizaciones[i].Moneda.toString() == "Dolar")
                document.getElementById("cotDolar").innerHTML = "<strong>" + data.Cotizaciones[i].Moneda.toString() + ":</strong> Compra: " + data.Cotizaciones[i].Compra.toString() + "  Venta: " + data.Cotizaciones[i].Venta.toString();

            if (data.Cotizaciones[i].Moneda.toString() == "Argentino")
                document.getElementById("cotPeso").innerHTML = "<strong>" + data.Cotizaciones[i].Moneda.toString() + ": </strong> Compra: " + data.Cotizaciones[i].Compra.toString() + " Venta: " + data.Cotizaciones[i].Venta.toString();

            if (data.Cotizaciones[i].Moneda.toString() == "Real")
                document.getElementById("cotReal").innerHTML =  "<strong>" + data.Cotizaciones[i].Moneda.toString() + ": </strong> Compra: " + data.Cotizaciones[i].Compra.toString() + " Venta: " + data.Cotizaciones[i].Venta.toString();

      }
       })
      .catch(console.log("Verificar que LiveServer este ejecutando para la URL: http://127.0.0.1:5500/PrimeraEntrega-JavaScript-main/js/cotizaciones.json"));
      document.getElementById("cotDolar").innerHTML = "Verificar que LiveServer este ejecutando para la URL:  http://127.0.0.1:5500/PrimeraEntrega-JavaScript-main/js/cotizaciones.json";
